...
if (accountLoggedIn()) { // Only if account is logged in
	if (isset($_SESSION["fileUploaded"],$_SESSION["fileDestination"])) {
		...
		if ($zip->extractTo($dest)) {
			...
			if (file_exists($file)) {
				if (!unlink($file)) {
					header("Location: ../../../?p=upl&msg=1&type=2");
					die("Redirecting");
				} else {
					...
					if (checkPatchAmount()) {
						...
						if ($conn->query($updatePatchCount)) {
							$conn->close();
							header("Location: ../../../?p=upl&msg=5");
							die("Redirecting");
						} else {
							header("Location: ../../../?p=upl&msg=9");
							die("Redirecting");
						}
					} else {
						header("Location: ../../../?p=upl&msg=10");
						die("Redirecting");
					}
				}
			} else {
			...
			header("Location: ../../../?p=upl&msg=8");
			die("Redirecting");
		}
	} else {
		header("Location: ../../../?p=upl&msg=7");
		die("Redirecting");
	}
} else {
	header("Location: ../../../");
